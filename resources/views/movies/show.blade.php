                        ...
                        @auth
                            <a href="#" class="btn btn-primary text-capitalize movie__fav-btn">
                                            <span class="far fa-heart movie__fav-icon {{ $movie->is_favored ? 'fw-900' : '' }} movie-{{ $movie->id }}"
                                                  data-movie-id="{{ $movie->id }}" data-url="{{ route('movies.toggle_favorite', $movie->id) }}"
                                            >
                                            </span>
                                add to favorite
                            </a>
                        ...
                        <div class="movie__details text-white">
                            ...
                            <div class="d-flex movie__cta">
                                <a href="{{ route('movies.show', $movie->id) }}"
                                   class="btn btn-primary text-capitalize flex-fill mr-2"><i class="fas fa-play"></i>watch now</a>
                                @auth
...
@push('scripts')
    ...
    <script>
        var file = "[Auto]{{ Storage::url('movies/'. $movie->id . '/' . $movie->id . '.m3u8') }}," +
